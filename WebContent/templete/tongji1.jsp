...
<body>
	...
	<form action="templete/tongji1.jsp" name="tongji1Form" method="get">
		<table style="width: 1100px;">
			<tr>
		...
		<table style="width: 1100px;">
			<tr>
				...
				<td style="width: 313px; text-align: left;">归口管理部门&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				 <select
				...
				<td style="text-align: left; width: 300px;">所在地域&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				 <select
			...
			<tr>
				...
				<td style="text-align: left;" colspan="2"><select name="sjsy"
					...
					id="tbTime1" style="width: 144px;" /> &nbsp;-&nbsp; <input
					type="text" name="tbTime2" id="tbTime2" style="width: 150px;" />&nbsp;&nbsp;&nbsp;&nbsp;

				</td>
				<td style="text-align: right;"><input type='submit'
					name='submit' id="submit" value='查询' class="btn"
					onclick="return Check1()" />&nbsp; <a href="templete/tongji.jsp"
					target="_self"><input type='button' name="button" id="button"
						value='显示全部' class="btn" /></a></td>
			</tr>
			<tr>
				<td
				...
				href='templete/tongji1.jsp?page=<%=intPage-1%>&shlx=<%=shlx%>&shzt=<%=shzt%>&OrgBMName=<%=OrgBMName%>&tbTime1=<%=tbTime1%>&tbTime2=<%=tbTime2%>&OrgPutunder=<%=OrgPutunder%>&OrgArea=<%=OrgArea%>&jgsx1=<%=jgsx1%>&jsxujjfs1=<%=jsxujjfs1%>&kjhdlx1=<%=kjhdlx1%>&sjsy=<%=sjsy%>&gjz=<%=gjz%>'>
					<span class="fenye">上一页</span>
	                 	...
	                 	for (int j = 1; j <= intPageCount; j++) {
	                 		out.print("&nbsp;<a href='templete/tongji1.jsp?page="+j+"&shlx="+shlx+"&shzt="+shzt+"&OrgBMName="+OrgBMName+"&tbTime1="+tbTime1+"&tbTime2="+tbTime2+"&OrgPutunder="+OrgPutunder+"&OrgArea="+OrgArea+"&jgsx1="+jgsx1+"&jsxujjfs1="+jsxujjfs1+"&kjhdlx1="+kjhdlx1+"&sjsy="+sjsy+"&gjz="+gjz+"'>"+j+"</a>");
	                 	}
	                 %>
			</span> <a
				href='templete/tongji1.jsp?page=<%=intPage+1%>&shlx=<%=shlx%>&shzt=<%=shzt%>&OrgBMName=<%=OrgBMName%>&tbTime1=<%=tbTime1%>&tbTime2=<%=tbTime2%>&OrgPutunder=<%=OrgPutunder%>&OrgArea=<%=OrgArea%>&jgsx1=<%=jgsx1%>&jsxujjfs1=<%=jsxujjfs1%>&kjhdlx1=<%=kjhdlx1%>&sjsy=<%=sjsy%>&gjz=<%=gjz%>'>
