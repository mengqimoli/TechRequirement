...
<%
		...
		if(rs11.next())
		{
			strUserName = rs11.getString("UserName");
			...
			strOrgTechNeed = rs11.getString("OrgTechNeed");
			strOrgNeedTime = rs11.getString("OrgNeedTime");
			strOrgNeedTime1 = rs11.getString("OrgNeedTime1");
			strOrgScienceNeed = rs11.getString("OrgScienceNeed");
			strOrgScienceNeed1 = rs11.getString("OrgScienceNeed1");
			strOrgScienceNeed2 = rs11.getString("OrgScienceNeed2");
			strOrgKey = rs11.getString("OrgKey");
			strOrgAllMoney = rs11.getString("OrgAllMoney");
			strOrgResearchType = rs11.getString("OrgResearchType");
			strOrgSubject = rs11.getString("OrgSubject");
			strOrgTechArea = rs11.getString("OrgTechArea");
			strOrgTechWork = rs11.getString("OrgTechWork");
			strOrgCooperation = rs11.getString("OrgCooperation");
			strOrgCooperUnit = rs11.getString("OrgCooperUnit");
		}
		rs11.close();
		...
		String strOrgXSSH = "";
		String strXSSHText = "";
		String strSql1 = "select * from XSSH where OrgNeedNum = '"+strOrgNeedNum+"'" ;
		ResultSet rs1= db.executeQuery(strSql1);
		while(rs1.next())
		{
			strOrgXSSH = rs1.getString("OrgXSSH");
			strXSSHText = rs1.getString("XSSHText");
		}
		rs1.close();
		
		String strBMSHText = "";
		String strSql2 = "select * from BMSH where OrgNeedNum = '"+strOrgNeedNum+"'" ;
		ResultSet rs2= db.executeQuery(strSql2);
		while(rs2.next())
		{
			strBMSHText = rs2.getString("BMSHText");
		}
		rs2.close();
...
<script type="text/javascript">
	...
	window.onload = function(){
	  ...
	  var bmsh = "<%=strOrgXSSH%>";
	  if(bmsh == "未通过"){
		  document.getElementById("bmshyj").style.display = "none"; 
	  }
	  //**************************************************************************************************回显复选框
...
*******************************************************************************************JAVA代码 -->
	...
	<table>
		...
		<tr>
			...
			<td colspan="3"><%=strXSSHText %></td>
		</tr>
		<tr id="bmshyj">
			<td style="height: 100px;">
				<div>部门审核意见</div>
			</td>
			<td colspan="3"><%=strBMSHText %></td>
		</tr>
	</table>
	<br />
	<div>
