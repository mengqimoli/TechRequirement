...
<%
	...
	if(rs11.next())
	{
		strUserName = rs11.getString("UserName").trim();
	...
	while(rs.next()){
		strOrgNum = rs.getString("OrgNum").trim();
		strOrgName = rs.getString("OrgName").trim();
		strOrgZip = rs.getString("OrgZip").trim();
	}
	rs.close();
%>
...
<script type="text/javascript">
	...
	function Check1()
	{
		var beizhu = document.getElementById("XSSHText").value;
		if(beizhu.trim()=="")
		{
			alert("请写明不通过的原因!");
			document.getElementById("XSSHText").focus();
			return false;
		}
		else
		{
...
<body>
	<h2 style="position: relative; left: -30px;">河北省重大技术需求征集表</h2>
	<!-- **********************************************************************************************************JAVA代码 -->
	...
	<form action="xsshServlet?OrgNeedNum=<%=strOrgNeedNum%>"
		id="xstableform" name="xstableform" method="post" target="_self">
		<table>
			<tr>
				<td style="height: 100px; width: 120px;">
					<div>形式审核意见</div>
				</td>
				<td colspan="3"><textarea cols="92" rows="6" id="XSSHText"
						name="XSSHText"></textarea></td>
			</tr>
		</table>
		<br />
		<div>
			管理处室:&nbsp; <select name="OrgBMName" id="OrgBMName"
				style="width: 200px">
				<%
								request.setCharacterEncoding("utf-8");
								
								String sql3="select GLCSName from GLCS";
								ResultSet rs3=db.executeQuery(sql3);
								while(rs3.next()){
								out.println("<option value="+rs3.getString(1)+">"+rs3.getString(1)+"</option>");
								}
								rs3.close();
							%>
			</select>
		...
		<br />
		<div>
			<input type="submit" name="submit" value="通过" class="btn" />
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="submit" name="submit"
				value="不通过" class="btn" onclick="return Check1()" />
		</div>
	</form>
	<br />
	<br />
